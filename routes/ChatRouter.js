...
ChatRouter.route('/')
    ...
    .post((req, res) => {
        //to create new chat
        const u1m = req.body.newChat.u1m,
            u2m = req.body.newChat.u2m;
        //get the users included whose details are included in request
        User.find({
                mobile: [u1m, u2m]
            }, '_id name mobile chats')
            .then((users) => {
                // console.log(users);
                ...
                if (typeof users[0] !== 'undefined' && typeof users[1] !== 'undefined') {
                    ...
                    Chat.findOne({
                            "u1.mobile": {
                                $in: [users[0].mobile, users[1].mobile]
                            },
                            "u2.mobile": {
                                $in: [users[0].mobile, users[1].mobile]
                            }
                    ...
                    res.setHeader('Content-Type', 'application/json');
                    res.json({
                        message: "Invalid Data, mobile no. not registered"
                    });
                }
            })
    })
...
ChatRouter.route('/:chatId')
    ...
    .post((req, res) => {
        ...
        Chat.findOne({
            ...
            .then((chat) => {
                // console.log(chat.u1, chat.u2);
                chat.messages.push(newMsg); //array of messages
                chat.save().then((msg) => {
                    res.setHeader('Content-Type', 'application/json');
                    res.json(msg);
                });
